package 평균은넘겠지_강사님0118;
import java.util.Scanner;

public class AboveArr {
    ...
    static double overRate() {
        Scanner sc = new Scanner(System.in);
        int cnt = sc.nextInt(); // 학생 수
        int total = 0; // 총점
        int overCnt = 0; // 평균을 넘는 학생 수
        int[] score = new int[cnt]; // 학생 수만큼 성적을 입력 받기 위한 배열 생성
        for (int i = 0; i < cnt; i++) {
            score[i] = sc.nextInt(); // 학생의 수만큼 성적을 입력받아 배열에 대입
            total += score[i]; // 학생의 수만큼 입력된 성적을 누적하여 총점을 구함
        }

...
// 자식의 생성자에서 부모의 생성자를 호출할 때는 ★★반드시 부모의 생성자를 먼저 호출하고 다른 초기화를 진행해야 함.

// 생성자를 따로 만들지 않으면 기본 생성자가 호출되는 것 처럼
// 부모의 생성자를 별도로 만들어주지 않으면 기본적으로 부모의 생성자가 자동 호출
...
class Child extends Parent {
    int b;
    Child() {
        super(200); // super는 자식 생성자가 실행되기 전에 호출해야 함.!!!!!!
